...
def run_single_experiment(
    ...
    semantic_scores = [r["semantic_score"] for r in results if r.get("semantic_score") is not None]
    ...
    print(
        f"Completed: OMA={summary.get('oma_accuracy', summary.get('accuracy', 0)):.3f}, "
...
def generate_sweetspot_bar_chart(
    ...
    bars2 = ax.bar(
        x + width / 2, tinyace_oma, width, label="TinyACE WM-512", color="#ff7f0e", alpha=0.8
    )
...
def main():
    ...
    seed = (
        ...
        else contract.get("dataset", {}).get("seed", DEFAULT_SEED)
    )
    set_seed(seed)
    environment = capture_environment()
    contract.setdefault("dataset", {})["seed"] = seed
    print(
        f"Seed: {seed} | torch={environment['torch_version']} "
